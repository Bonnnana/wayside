...
fun WelcomeIllustration(
    ...
    Canvas(modifier) {
        ...
        // The road, and the two pins that make it a Wayside road.
        drawWindingRoad(
        ...
        listOf(
            PlaceCategory.Food.color to Offset(w * 0.29f, h * 0.48f),
            PlaceCategory.Viewpoints.color to Offset(w * 0.37f, h * 0.335f),
        ).forEach { (pinColor, center) ->
            drawCircle(color = Color.White, radius = w * 0.028f, center = center)
            drawCircle(color = pinColor, radius = w * 0.019f, center = center)
        }
    }
}
